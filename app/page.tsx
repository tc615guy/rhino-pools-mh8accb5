...
export default function Page() {
  // Set default language based on user's choice: Spanish → 'es', otherwise 'en'
  ...
  const businessName = "Rhino Pools";
  const serviceType = "Pool Installation and Maintenance ";
  const serviceDescription = "Full Pool Design and Installation, Pool Maintenance ";
            ...
            <div className="text-base sm:text-lg text-gray-600 mb-6 sm:mb-8">
              {serviceDescription.includes(',') ? (
                <ul className="space-y-2">
                  {serviceDescription.split(',').map((service, index) => (
                    <li key={index} className="flex items-start">
                      ...
                      <span className="leading-relaxed">{service.trim()}</span>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="leading-relaxed">{serviceDescription}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <div className="flex space-x-4">
                <a href="Https://facebook/pools" target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition-colors">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </a>
                <a href="Https://instagram/pools" target="_blank" rel="noopener noreferrer" className="text-gray-300 hover:text-white transition-colors">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
